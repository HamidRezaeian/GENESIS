...
def _write_atomic(path, arrays):
    """np.savez to a temp file, then update `path` GUARANTEED under all circumstances."""
    import shutil
    ...
    try:
        ...
        # Stage 2: Direct overwrite via binary stream copy (overcomes open-reader Windows locks)
        if not written:
                ...
                try:
                    if os.path.exists(path):
                        try:
                            ...
                            pass
                    with open(tmp, 'rb') as f_src, open(path, 'wb') as f_dst:
        ...
        # Stage 3: Fallback using shutil.copyfile
        if not written:
            for attempt in range(5):
                try:
        ...
        if not written:
            try:
                ...
                raise OSError(f"CRITICAL ERROR: Unable to update {path} under any fallback method: {final_err}")
    finally:
...
def save_brain(path, hall_of_fame, saved_at_tick, keep_k):
    ...
    keep_k      : max champions to retain on disk (default FOSSIL_POOL_MAX).

    Returns (saved_count:int, archived_legacy_filename:str or None).
    """
    cur = current_fingerprint()
    archived = None
