# lava_network_v4.py - با ports صحیح hybrid
"""
GENESIS Phase 2 — Complete Event-Driven Network (v4)
Source(spike) -> Dense(spike->analog) -> LIF(analog->spike) -> Sink(spike)
"""
...
def main():
    print("=" * 70)
    print("GENESIS Phase 2 — Complete Event-Driven Network (v4)")
    print("=" * 70)
    ...
    n_steps = 500
    input_sparsity = 0.05
    synaptic_weight = 2  # integer for Loihi compatibility

    print(f"\n🧠 Network: {n_neurons} neurons, {n_steps} steps")

    # Create input data: sparse random spikes
    np.random.seed(42)
    # Shape: [n_neurons, n_steps] - each row is one neuron's spike train
    ...
    # num_message_bits=0 means spike-based (no payload)
    ...
    dense = Dense(weights=weights, num_message_bits=0)  # spike-based
    
    # LIF: neurons (analog input -> spike output)
    lif = LIF(
        shape=(n_neurons,),
        u=0,
        v=0,
    ...
    # Connect with CORRECT hybrid ports
    source.s_out.connect(dense.s_in)   # spike -> spike
    dense.a_out.connect(lif.a_in)      # analog -> analog (synaptic current)
    lif.s_out.connect(sink.s_in)       # spike -> spike
    
    print(f"   ✅ Source.s_out -> Dense.s_in (spike)")
    print(f"   ✅ Dense.a_out -> LIF.a_in (synaptic current)")
    print(f"   ✅ LIF.s_out -> Sink.s_in (spike)")
    print(f"   ✅ Synaptic connections: {int(np.sum(connectivity_mask)):,} ({np.sum(connectivity_mask)/(n_neurons*n_neurons):.1%} connectivity)")
