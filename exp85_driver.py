"""
Experiment 85: Deep-Time Ascension Scaling Benchmark Driver (10,000 Ticks, Rule 3, Rule 8, Rule 18, Rule 20)
================================================================================-------------------------
Runs a 5-seed x 2-arm A/B benchmark across 10,000 continuous ticks in the Phase D Grounded Ecology.
...
ARMS = [
    {
        "name": "Arm 1: Control (10k Phase D Reflex)",
        "env": {
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
            "GENESIS_REFUGE": "1",
    ...
    {
        "name": "Arm 2: Proposed (10k Multi-Timescale SNN)",
        "env": {
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
            "GENESIS_REFUGE": "1",
...
def main():
    worker_script = os.path.join(REPO_ROOT, "exp85_worker.py")
    with open(worker_script, "w", encoding="utf-8") as f:
        f.write(SUBWORKER_CODE)

    print("==========================================================================")
    print("Experiment 85: Deep-Time Ascension 10,000-Tick Benchmark (Rule 3)")
    print("==========================================================================")
    ...
    for arm_name, seed_runs in results.items():
        ...
        print(f"{arm_name:<40}: Mean Pop = {arm_stats[arm_name]['mean_pop']:.1f} ± {arm_stats[arm_name]['std_pop']:.1f} | Mean Energy = {arm_stats[arm_name]['mean_energy']:.1f} ± {arm_stats[arm_name]['std_energy']:.1f}")

    arm1_pop = arm_stats["Arm 1: Control (10k Phase D Reflex)"]["mean_pop"]
    arm2_pop = arm_stats["Arm 2: Proposed (10k Multi-Timescale SNN)"]["mean_pop"]
    arm2_std = arm_stats["Arm 2: Proposed (10k Multi-Timescale SNN)"]["std_pop"]
    ...
    verdict = "ASCENT CONFIRMED (Selection Advantage >= 1.0σ)" if passed else "NULL / FALSIFIED (Selection Advantage < 1.0σ)"
    print(f"BINDING VERDICT (Rule 2 / Rule 18): {verdict}")

    out_file = os.path.join(REPO_ROOT, "exp85_results.json")
    summary = {
        "arms": results,
        "stats": arm_stats,
        "delta_pop": delta_pop,
