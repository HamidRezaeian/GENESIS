"""
Experiment 90: In-Lifetime Learning Selection under Non-Stationary Environment (REMAP)
(Rule 2, Rule 3, Rule 6, Rule 7, Rule 14, Rule 16, Rule 18, Rule 20)
================================================================================================
Tests whether plasticity (STDP3C + MULTISCALE SNN) provides a statistically significant selection
advantage (Z >= 1.0σ) when the environment is non-stationary (GENESIS_REMAP=1).

Arm 1 (Control):  REMAP=1, AUTO_REPRO=1, FOOTPRINT=1, MULTISCALE=0, STDP3C=0 (Fixed Reflex)
Arm 2 (Proposed): REMAP=1, AUTO_REPRO=1, FOOTPRINT=1, MULTISCALE=1, STDP3C=1 (Plastic Learner)
...
ARMS = [
    {
        "name": "Arm 1: Control (REMAP, Fixed Reflex, No Plasticity)",
        "env": {
            "GENESIS_REMAP": "1",
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
    ...
    {
        "name": "Arm 2: Proposed (REMAP, Plastic MULTISCALE+STDP3C)",
        "env": {
            "GENESIS_REMAP": "1",
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
...
def main():
    worker_script = os.path.join(REPO_ROOT, "exp90_worker.py")
    with open(worker_script, "w", encoding="utf-8") as f:
        f.write(SUBWORKER_CODE)

    print("==========================================================================")
    print("Experiment 90: In-Lifetime Learning Selection under Non-Stationary REMAP")
    print("==========================================================================")
    ...
    arm1 = arm_stats["Arm 1: Control (REMAP, Fixed Reflex, No Plasticity)"]
    arm2 = arm_stats["Arm 2: Proposed (REMAP, Plastic MULTISCALE+STDP3C)"]
    ...
    if passed:
        verdict = "🟢 ASCENT CONFIRMED: Genuine In-Lifetime Learning Selection Advantage (Z >= 1.0σ under REMAP)"
    else:
        verdict = f"VERDICT: ΔPop Z={z_pop:+.2f}σ, ΔBirths={delta_births:+.1f}"

    print(f"BINDING VERDICT: {verdict}")

    out_file = os.path.join(REPO_ROOT, "exp90_results.json")
