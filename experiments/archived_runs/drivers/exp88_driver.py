"""
Experiment 88: Grounded Selection Gradient under Footprint Economy (AUTO_REPRO + INCOME_FOOTPRINT)
(Rule 2, Rule 3, Rule 6, Rule 14, Rule 16, Rule 18, Rule 20)
================================================================================================
Evaluates multi-generational selection when the metabolic ceiling is opened by
GENESIS_INCOME_FOOTPRINT=1 (FOOTPRINT_QUANTUM=898.0 energy/byte > 703.6 base drain)
combined with GENESIS_AUTO_REPRO=1 (energy-gated physical fission threshold=200,000).

Arm 1 (Control):  INCOME_FOOTPRINT=1, AUTO_REPRO=1, MULTISCALE=0, STDP3C=0 (No Plasticity)
Arm 2 (Proposed): INCOME_FOOTPRINT=1, AUTO_REPRO=1, MULTISCALE=1, STDP3C=1 (Multi-Scale + Learning)

5 seeds x 10,000 ticks.
Measures: Mean Pop, Mean Energy, Total Births, Birth Rate, Z-score (Rule 18 finish line).
"""
...
ARMS = [
    {
        "name": "Arm 1: Control (Footprint + AutoRepro, No Plasticity)",
        "env": {
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
    ...
    {
        "name": "Arm 2: Proposed (Footprint + AutoRepro, MULTISCALE+STDP3C)",
        "env": {
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
            "GENESIS_INCOME_FOOTPRINT": "1",
...
for org_idx in range(50):
    spawn_organism(org_idx, (org_idx * 1200) % RAM_SIZE, ancestor, 100000)
...
def main():
    worker_script = os.path.join(REPO_ROOT, "exp88_worker.py")
    with open(worker_script, "w", encoding="utf-8") as f:
        f.write(SUBWORKER_CODE)

    print("==========================================================================")
    print("Experiment 88: Grounded Selection Gradient under Footprint Economy")
    print("==========================================================================")
    ...
    arm1 = arm_stats["Arm 1: Control (Footprint + AutoRepro, No Plasticity)"]
    arm2 = arm_stats["Arm 2: Proposed (Footprint + AutoRepro, MULTISCALE+STDP3C)"]
    ...
    births_nonzero = arm1["mean_births"] > 0 or arm2["mean_births"] > 0
    passed = (delta_pop > 0) and (z_pop >= 1.0) and (arm2["mean_births"] > arm1["mean_births"])

    if passed:
        verdict = "MULTI-GENERATIONAL ASCENT CONFIRMED (ΔPop Z≥1σ under Footprint Economy)"
    else:
        verdict = f"VERDICT: ΔPop Z={z_pop:+.2f}σ, ΔBirths={delta_births:+.1f}"

    print(f"BINDING VERDICT: {verdict}")

    out_file = os.path.join(REPO_ROOT, "exp88_results.json")
