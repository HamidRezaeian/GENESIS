"""
Experiment 82: Multi-Timescale SNN Dynamics Driver (Rule 3, Rule 8, Rule 18, Rule 20)
========================================================================================
Runs a 5-seed x 2-arm A/B benchmark across separate sub-processes to isolate Numba JIT cache.

Arm 1 (Control): Single-timescale LIF baseline (GENESIS_MULTISCALE=0)
Arm 2 (Proposed): Multi-timescale SNN (GENESIS_MULTISCALE=1, tau_slow=25.0)

Measures: Mean, Std, Delta (Arm 2 vs Arm 1), and Z-score across 5 independent seeds.
"""
...
ARMS = [
    {"name": "Arm 1: Control (Single-Timescale)", "env": {"GENESIS_MULTISCALE": "0"}},
    {"name": "Arm 2: Proposed (Multi-Timescale)", "env": {"GENESIS_MULTISCALE": "1"}},
]
...
def main():
    worker_script = os.path.join(REPO_ROOT, "exp82_worker.py")
    with open(worker_script, "w", encoding="utf-8") as f:
        f.write(SUBWORKER_CODE)

    print("==========================================================================")
    print("Experiment 82: Multi-Timescale SNN Dynamics Multi-Seed Driver (Rule 3)")
    print("==========================================================================")
    ...
    for arm_name, seed_runs in results.items():
        ...
        print(f"{arm_name:<30}: Mean Pop = {arm_stats[arm_name]['mean_pop']:.1f} ± {arm_stats[arm_name]['std_pop']:.1f} | Mean Energy = {arm_stats[arm_name]['mean_energy']:.1f} ± {arm_stats[arm_name]['std_energy']:.1f}")

    arm1_pop = arm_stats["Arm 1: Control (Single-Timescale)"]["mean_pop"]
    arm2_pop = arm_stats["Arm 2: Proposed (Multi-Timescale)"]["mean_pop"]
    arm2_std = arm_stats["Arm 2: Proposed (Multi-Timescale)"]["std_pop"]
    ...
    out_file = os.path.join(REPO_ROOT, "exp82_results.json")
    summary = {
        "arms": results,
        "stats": arm_stats,
        "delta_pop": delta_pop,
