"""Substrate 4 Non-Stationary Task Driver.

Protocol ID: SUBSTRATE_4_NONSTATIONARY_TASK_v1

Evaluates Substrate 4 (Small Transformer) on a non-stationary sequence where
the underlying pattern changes every 5000 ticks (4 distinct text patches in 20,000 ticks).

Goal: Test if task saturation masked in-lifetime adaptation capability.

Outputs:
  - experiments/sub4_results/sub4_nonstationary_summary.json
"""
...
def generate_patterns(seed):
    p1 = [ord(c) for c in ("the quick brown fox jumps over the lazy dog 0123456789 ") * 10][:PATCH_SIZE]
    p2 = [ord(c) for c in ("to be or not to be that is the question whether tis nobler ") * 10][:PATCH_SIZE]
    p3 = [ord(c) for c in ("def world_tick_numba(ram, orgs, atp, weights, synapses): ") * 10][:PATCH_SIZE]
    p4 = [ord(c) for c in ("biological neural systems learn continuously within lifetime ") * 10][:PATCH_SIZE]
    return [p1, p2, p3, p4]
...
def run_arm(seed, is_learn):
    ...
    for tick in range(TICKS):
        pattern_idx = (tick // SWITCH_EVERY) % len(patterns)
        patch = patterns[pattern_idx]
        n = len(patch)
...
def main():
    ...
    print("=" * 64)
    print("SUBSTRATE 4 NON-STATIONARY TASK RUN")
    print(f"Seeds: {SEEDS} | Ticks: {TICKS} | Switch Every: {SWITCH_EVERY} | Orgs: {N_ORGS}")
    print("=" * 64)

    summary = {
        "substrate": "Substrate4_Nonstationary_Task",
        "protocol": "SUBSTRATE_4_NONSTATIONARY_TASK_v1",
        "seeds": SEEDS,
        "ticks": TICKS,
    ...
    out_file = os.path.join(results_dir, "sub4_nonstationary_summary.json")
    with open(out_file, "w") as f:
        json.dump(summary, f, indent=2)

    print("\n" + "=" * 64)
    print("SUBSTRATE 4 NON-STATIONARY SUMMARY & VERDICT")
    print("=" * 64)
