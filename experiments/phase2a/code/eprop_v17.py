#!/usr/bin/env python
"""
Exp-P2A-01 v17: ELIGIBILITY IN EVERY TICK (FINAL FIX)
======================================================
مشکل v16: Eligibility traces فقط در T_out update می‌شدند.
Fix: Eligibility در هر tick update شود، weight update فقط در T_out.
"""
...
class SynapticLayer:
    ...
    def update_eligibility(self, pre_spikes, post_f_prime, tau_e):
        """Eligibility update: ε(t) = decay × ε(t-1) + f'(v) × z(t-1)"""
...
def run_organism(arm, input_pulses, targets, config, seed):
    """Run with eligibility in EVERY tick."""
    ...
    for t in range(len(input_pulses)):
        ...
        # ← FIX: Eligibility update in EVERY tick (not just T_out)
        ...
        if targets[t] != -1:
            ...
            if arm in ['A1_eprop', 'A3_stdp3c']:
                ...
                # Hidden layer updates (using accumulated eligibility)
...
def main():
    print("=" * 70)
    print("Exp-P2A-01 v17: ELIGIBILITY IN EVERY TICK")
    print("=" * 70)
    print(f"Task: Temporal XOR with trial structure")
    print(f"Key fix: Eligibility traces update in EVERY tick")
    ...
    filename = RESULTS_DIR / f'exp_p2a_01_v17_every_tick_{timestamp}.json'
    
    data = {
        'timestamp': timestamp,
        'version': 'v17 (eligibility every tick)',
        'config': CONFIG,
        'results': results,
