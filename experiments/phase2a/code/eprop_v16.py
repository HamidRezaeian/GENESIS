#!/usr/bin/env python
"""
Exp-P2A-01 v16: FINAL FIXES
============================
مشکلات v15:
1. Weight init خیلی کوچک (0.1) → hidden spike نمی‌زند
2. Threshold خیلی بالا (0.5) → سخت برای spike
3. dW_out با z در یک لحظه → اغلب صفر

Fixes:
1. w_scale = 1.0 (بزرگ‌تر)
2. theta = 0.3 (پایین‌تر)
3. Readout window: average spikes over last 5 ticks
4. Input encoding قوی‌تر (10 neurons per bit)
"""
...
CONFIG = {
    ...
    # Output
    'tau_out': 20.0,
    'readout_window': 5,  # ← FIX: average over last 5 ticks
    
    # Synaptic
    'w_scale': 1.0,      # ← FIX: larger weights
    'connectivity': 0.3,  # ← FIX: more connections
...
def encode_input(pulse, input_t3, n_input):
    """Stronger encoding: 10 neurons per bit."""
    x = np.zeros(n_input)
    if pulse == 1:
        x[:10] = 1.0  # ← 10 neurons fire
    if input_t3 == 1:
        x[10:20] = 1.0  # ← 10 neurons fire
    return x
...
def run_organism(arm, input_pulses, targets, config, seed):
    ...
    for t in range(len(input_pulses)):
        ...
        # ← FIX: Track hidden spikes for readout window
        ...
        # Error mask: only at T_out positions
        if targets[t] != -1:
            target = targets[t]
            ...
            if arm in ['A1_eprop', 'A3_stdp3c']:
                ...
                # ← FIX: Output layer update with READOUT WINDOW
                # Average hidden spikes over last readout_window ticks
                ...
                hidden_spike_history = []  # ← FIX: also reset history
...
def main():
    print("=" * 70)
    print("Exp-P2A-01 v16: FINAL FIXES")
    print("=" * 70)
    print(f"Task: Temporal XOR with trial structure")
    print(f"Fixes applied:")
    print(f"  1. w_scale = {CONFIG['w_scale']} (larger weights)")
    print(f"  2. theta = {CONFIG['theta']} (lower threshold)")
    print(f"  3. Readout window = {CONFIG['readout_window']} ticks")
    print(f"  4. Input encoding: 10 neurons per bit")
    ...
    if delta >= 5.0 and p_value < 0.05:
        ...
        print("  ⚠️  H1 NULL")
        if a1['mean_loss'] < 0.69:
            print("  → Loss decreasing but accuracy not improving")
        else:
            print("  → No learning detected")
    
    # Save
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = RESULTS_DIR / f'exp_p2a_01_v16_final_{timestamp}.json'
    
    data = {
        'timestamp': timestamp,
        'version': 'v16 (final fixes)',
        'config': CONFIG,
        'results': results,
