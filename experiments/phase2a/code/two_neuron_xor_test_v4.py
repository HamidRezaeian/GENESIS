#!/usr/bin/env python
"""
2-Neuron XOR Test v4 (FIXED)
=============================
Fix: Accumulate hidden activity over trial for dW_out
"""

import numpy as np

def two_neuron_xor_test_v4():
    """XOR with accumulated hidden activity."""
    
    print("=" * 70)
    print("2-Neuron XOR Test v4 (FIXED)")
    print("=" * 70)
    print("Network: 2 input → 5 hidden LIF → 2 output (softmax)")
    print("Task: XOR of two bits presented at t=1 and t=2")
    print("Fix: Accumulate hidden spikes over trial for dW_out")
    print("=" * 70)
    ...
    for epoch in range(300):
        ...
        for inp, target_class in xor_data:
            ...
            # ← FIX: Accumulate hidden activity
            ...
            # ← FIX: dW_out uses accumulated hidden activity
            dW_out += np.outer(error, h_activity)  # error: (2,), h_activity: (5,) → (2, 5) ✓
    ...
    if final_acc > 0.8:
        ...
        print(f"   → Accumulated hidden activity fixes the issue")
