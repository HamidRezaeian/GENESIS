#!/usr/bin/env python
"""
Exp-P2A-01 v15: Four Key Fixes from Opus (FIXED)
=================================================
Fix broadcasting error in W_out update
"""
...
class LIFPool:
    def __init__(self, n, theta=0.5, dv=0.05, du=0.05):
        self.n = n
        self.theta = theta
        self.dv = dv
        self.du = du
    ...
    for t in range(len(input_pulses)):
        ...
        if targets[t] != -1:
            ...
            if arm in ['A1_eprop', 'A3_stdp3c']:
                ...
                # ← FIXED: Output layer update with s_hidden (not r_out)
                # r_out is computed from W_out, so we can't use it to update W_out
                # Instead, use hidden spikes directly
                delta_out = eta * np.outer(error, s_hidden)  # error: (2,), s_hidden: (100,) → (2, 100)
...
def main():
    print("=" * 70)
    print("Exp-P2A-01 v15: Four Key Fixes (FIXED broadcasting)")
    print("=" * 70)
    print(f"Task: Temporal XOR with trial structure")
    print(f"Fixes applied:")
    print(f"  1. eta = {CONFIG['eta']} (Bellec 2020)")
    print(f"  2. Error mask: only at T_out={CONFIG['T_out']}")
    print(f"  3. W_out update with s_hidden (FIXED: not r_out)")
    print(f"  4. Full state reset between trials")
    ...
    filename = RESULTS_DIR / f'exp_p2a_01_v15_fixed_{timestamp}.json'
    
    data = {
        'timestamp': timestamp,
        'version': 'v15 (four key fixes, broadcasting fixed)',
        'config': CONFIG,
        'results': results,
