#!/usr/bin/env python
"""
Exp-P2A-01 v21: Full Temporal XOR با Correct Sign + Stability
===============================================================
Fixes از 2-neuron test:
1. Sign invert: W += eta * dW
2. eta = 0.01
3. Gradient clipping
4. Weight clipping
"""
...
class SynapticLayer:
    ...
    def apply_update_signed(self, learning_signal, eta, max_weight=5.0, max_grad=1.0):
        """← KEY FIX: SIGN INVERT
        learning_signal شامل L_hidden است (بدون علامت منفی)
        W += eta * L * eps (not -=)
        """
        learning_signal = np.clip(learning_signal, -max_grad, max_grad)
        # ← SIGN INVERT: += instead of -=
        delta_w = eta * np.outer(learning_signal, np.ones(self.n_in)) * self.eligibility
        delta_w = np.clip(delta_w, -max_grad, max_grad)
        self.weights += delta_w  # ← += (sign invert)
    ...
    for t in range(len(input_pulses)):
        ...
        if targets[t] != -1:
            ...
            if arm in ['A1_buffered', 'A3_coupled']:
                ...
                # ← KEY FIX: SIGN INVERT
                # L_hidden = W_out^T @ error (WITHOUT negative sign)
                # because we use += for update
                L_hidden = syn_out.weights.T @ error  # ← no negative!
                ...
                if can_update:
                    # ← KEY FIX: SIGN INVERT (apply_update_signed uses +=)
                    syn_in.apply_update_signed(L_hidden, eta, 
                                               config['max_weight'], config['max_grad'])
                    syn_rec.apply_update_signed(L_hidden, eta, 
                                                config['max_weight'], config['max_grad'])
                    ...
                    # ← KEY FIX: SIGN INVERT for output layer
                    # error = y - target, so for gradient descent: W += eta * error * h
                    delta_out = eta * np.outer(error, s_hidden_avg)  # ← no negative!
                    delta_out = np.clip(delta_out, -config['max_grad'], config['max_grad'])
                    syn_out.weights += delta_out  # ← += (sign invert)
...
def main():
    print("=" * 70)
    print("Exp-P2A-01 v21: Full Temporal XOR with Correct Sign")
    print("=" * 70)
    print("Key fixes from 2-neuron test:")
    print(f"  1. Sign invert: W += eta * L * eps (not -=)")
    print(f"  2. eta = {CONFIG['eta']} (smaller)")
    print(f"  3. Gradient clipping (max={CONFIG['max_grad']})")
    print(f"  4. Weight clipping (max={CONFIG['max_weight']})")
    ...
    filename = RESULTS_DIR / f'exp_p2a_01_v21_sign_fix_{timestamp}.json'
    
    data = {
        'timestamp': timestamp,
        'version': 'v21 (sign fix from 2-neuron test)',
        'config': CONFIG,
        'results': results,
