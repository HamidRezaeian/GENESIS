#!/usr/bin/env python
"""
Exp-P2A-01 v22: The Definitive Fix
===================================
باگ‌های کشف شده در v21 که در اینجا رفع شده‌اند:
1. SIGN ERROR (مهم‌ترین): v21 از `W += delta_w` استفاده می‌کرد که منجر به Gradient Ascent می‌شد. 
   اکنون از `W -= delta_w` (استاندارد Gradient Descent) استفاده می‌کنیم.
2. TASK MISMATCH: شبکه در v21 فقط `pulses[t]` و `pulses[t-3]` را می‌دید، اما Target برابر بود با 
   `pulses[t-3] ^ pulses[t-6]`. شبکه `pulses[t-6]` را هرگز نمی‌دید!
   اکنون `pulses[t-T1]` و `pulses[t-T2]` به عنوان ورودی به شبکه داده می‌شوند.
"""
...
CONFIG = {
    'n_ticks': 3000,
    'T1': 3,   # فاصله زمانی اولین ورودی
    'T2': 6,   # فاصله زمانی دومین ورودی
    'T_out': 9,# زمان تولید خروجی (Target)
...
def encode_input(pulse_t, pulse_t_T1, pulse_t_T2, n_input):
    """
    اصلاح v22: اضافه کردن pulse_t_T2 به ورودی شبکه
    """
    x = np.zeros(n_input)
    if pulse_t == 1:
        x[:10] = 1.0
...
def run_organism(arm, input_pulses, targets, config, seed):
    ...
    for t in range(len(input_pulses)):
        # اصلاح v22: دریافت درست ورودی‌ها
        pulse_t = input_pulses[t]
        pulse_t_T1 = input_pulses[t - T1] if t >= T1 else 0
        pulse_t_T2 = input_pulses[t - T2] if t >= T2 else 0
        
        x = encode_input(pulse_t, pulse_t_T1, pulse_t_T2, config['n_input'])
        ...
        if targets[t] != -1:
            ...
            if arm in ['A1_buffered', 'A3_coupled']:
                ...
                # اصلاح v22: محاسبه L_hidden (بدون تغییر علامت اضافی)
                ...
                if can_update:
                    # اصلاح v22: apply_update اکنون از -= استفاده می‌کند
...
def main():
    print("=" * 70)
    print("Exp-P2A-01 v22: The Definitive Fix (Sign + Task Mismatch)")
    print("=" * 70)
    print("Fixes applied:")
    print("  1. SIGN ERROR: Changed `W += delta_w` to `W -= delta_w` (Standard Gradient Descent)")
    print("  2. TASK MISMATCH: Network now receives pulses[t], pulses[t-T1], and pulses[t-T2]")
    ...
    filename = RESULTS_DIR / f'exp_p2a_01_v22_definitive_fix_{timestamp}.json'
    
    data = {
        'timestamp': timestamp,
        'version': 'v22 (Sign + Task Mismatch Fix)',
        'config': CONFIG,
        'results': results,
