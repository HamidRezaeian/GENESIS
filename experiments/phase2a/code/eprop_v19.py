#!/usr/bin/env python
"""
Exp-P2A-01 v19: REAL Buffering Implementation
==============================================
اولین experiment که واقعاً A1 ≠ A3 را test می‌کند.

A1 (Buffered):
  - Plasticity pool جدا از survival pool
  - Gate: اگر pool خالی است، update نمی‌شود
  - Eligibility trace banking

A3 (Coupled):
  - همیشه update (بدون gate)
  - plasticity و survival از یک pool

A2 (NOLEARN):
  - هیچ update

این اولین experiment valid است که D5 prediction را test می‌کند.
"""
...
CONFIG = {
    ...
    # Learning
    'eta': 0.05,      # ← Higher (Opus recommendation)
    'tau_e': 20.0,
    'beta': 5.0,
    ...
    'n_seeds': 8,  # More seeds for statistical power
    'random_seed': 42,
}
...
class SynapticLayer:
    ...
    def apply_update(self, learning_signal, eta, max_weight=5.0, max_grad=1.0):
        """ΔW = η × L × ε (learning_signal شامل sign منفی است)"""
...
class PlasticityBuffer:
    """
    Pool جداگانه برای plasticity.
    
    - capacity: حداکثر مقدار pool
    - refill_rate: نرخ پر شدن (بر اساس neuromodulatory signal)
    - update_cost: هزینه هر weight update
    """
    def __init__(self, capacity=50.0, refill_rate=0.5, update_cost=1.0):
        self.pool = capacity / 2  # شروع از نصف
    ...
    def refill(self, M_t):
        """Pool پر می‌شود بر اساس neuromodulatory signal."""
        if M_t > 0:
...
def run_organism(arm, input_pulses, targets, config, seed):
    """Run with REAL buffering difference between A1 and A3."""
    ...
    for t in range(len(input_pulses)):
        ...
        if targets[t] != -1:
            ...
            if arm in ['A1_buffered', 'A3_coupled']:
                # ← KEY: M_t from prediction error
                M_t = float(np.max(np.abs(error)))  # neuromodulatory signal
                
                # ← KEY: Buffer refill (only for A1)
    ...
    if buffer is not None:
        buffer_stats = {
            ...
            'block_rate': buffer.blocked_updates / max(1, buffer.blocked_updates + buffer.total_updates),
        }
...
def main():
    print("=" * 70)
    print("Exp-P2A-01 v19: REAL Buffering Implementation")
    print("=" * 70)
    print(f"Task: Temporal XOR (T1={CONFIG['T1']}, T2={CONFIG['T2']})")
    print(f"Arms:")
    print(f"  A1 (Buffered):  Plasticity pool with gate")
    print(f"  A2 (NOLEARN):   No learning")
    print(f"  A3 (Coupled):   Always updates (no gate)")
    print(f"eta = {CONFIG['eta']} (increased from 0.01)")
    ...
    for arm in CONFIG['arms']:
        ...
        for seed_idx in range(CONFIG['n_seeds']):
            ...
            if result['buffer_stats']:
                buf_info = f", blocked={result['buffer_stats']['blocked_updates']}, pool={result['buffer_stats']['final_pool']:.1f}"
    ...
    if delta_buffer > 2.0:
        print(f"  ✅ D5 PREDICTION CONFIRMED: buffering helps!")
        print(f"     Learning under constraint works when buffered")
    elif delta_buffer < -2.0:
        print(f"  ❌ D5 FALSIFIED: buffering hurts!")
    else:
        print(f"  ⚠️  D5 inconclusive: small difference")
    
    # Check if A1 is gated
    print()
    print("Buffer Activity Check:")
    print("-" * 60)
    print(f"  A1 blocked updates: {a1['total_blocked']}")
    print(f"  A1 successful updates: {a1['total_updates']}")
    if a1['total_blocked'] > 0:
        print(f"  ✅ Buffer is ACTIVE (gating happens)")
        block_rate = a1['total_blocked'] / max(1, a1['total_blocked'] + a1['total_updates'])
        print(f"  Block rate: {block_rate:.2%}")
    else:
        print(f"  ⚠️  Buffer never blocked — gate not constraining")
        print(f"     → Increase update_cost or decrease refill_rate")
    
    # Phase 1 reproduction
    print()
    print("Phase 1 Reproduction Check (learning hurts?):")
    print("-" * 60)
    ...
    if delta_a3_a2 < -2.0:
        ...
        print(f"  ❌ Phase 1 NOT reproduced: learning helps")
    else:
        print(f"  ⚠️  Inconclusive: no significant difference")
    
    # Gate A (primary metric)
    print()
    ...
    if delta_buffer > 2.0 and p_a1_a3 < 0.05:
        print("  🎉 H1 CONFIRMED + D5 VALIDATED!")
        print("     Buffering rescues learning under metabolic constraint")
    elif delta_gate > 5.0 and p_a1_a2 < 0.05:
        print("  🎉 Gate A PASSED: buffered e-prop beats nolearn")
        print("     (D5 effect size small but positive)")
    else:
        print("  ⚠️  H1 NULL or INCONCLUSIVE")
        if delta_a3_a2 < -2.0:
            print("  ✅ Phase 1 reproduced (learning hurts without buffering)")
            print("  ❌ But buffering did not rescue (needs stronger buffer?)")
    
    # Save
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = RESULTS_DIR / f'exp_p2a_01_v19_buffered_{timestamp}.json'
    
    data = {
        'timestamp': timestamp,
        'version': 'v19 (real buffering)',
        'config': CONFIG,
        'results': results,
        ...
        'd5_confirmed': bool(delta_buffer > 2.0 and p_a1_a3 < 0.05),
        'phase1_reproduced': bool(delta_a3_a2 < -2.0),
    }
