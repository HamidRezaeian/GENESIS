"""Substrate 5 — Hybrid Transformer + Sparse Event-Driven Memory.

Protocol ID: SUBSTRATE_5_HYBRID_TRANSFORMER_MEMORY_v1

Architecture:
  - Transformer Core: Small Causal Transformer (d_model=32, L=16 context)
  - Sparse Event Memory (CAM-style Key-Value Memory):
      * Capacity: 64 sparse key-value slots per organism
      * Key: Context representation vector h_t (d_model)
      * Value: Target byte x_{t+1}
      * Dynamic Write: Memory stores patterns on high-error events
      * Gated Retrieval: If max_sim(h_t, K_m) > 0.85, memory output overrides prediction

Evaluation Protocol:
  - Cohort: 60 organisms
  - Duration: 2000 ticks per seed, reported every 200 ticks
  - Seeds: [0, 1, 2, 3] (4 seeds)
  - Arms: LEARN (active hybrid learning) vs NOLEARN (frozen base model)

Outputs:
  - experiments/sub5_results/sub5_summary.json
"""
...
from sub4_small_transformer import SmallTransformerAgent, build_patch, softmax

class HybridAgent(SmallTransformerAgent):
    """Hybrid Transformer + Sparse Event-Driven Memory Agent."""
    def __init__(self, seed):
        super().__init__(seed)
        # Sparse Memory Bank
        self.mem_keys = np.zeros((MEMORY_SLOTS, D_MODEL), dtype=np.float32)
        self.mem_vals = np.full(MEMORY_SLOTS, -1, dtype=np.int32)
        self.mem_valid = np.zeros(MEMORY_SLOTS, dtype=bool)
        self.mem_ptr = 0
    ...
    def step(self, in_byte, tgt_byte, is_learn):
        ...
        # Check Sparse Event Memory Lookup
        mem_override = False
        final_pred = base_pred

        if np.any(self.mem_valid):
            valid_indices = np.where(self.mem_valid)[0]
            valid_keys = self.mem_keys[valid_indices]
            
            # Cosine similarity between current context last_h and stored keys
            h_norm = np.linalg.norm(last_h) + 1e-8
            k_norms = np.linalg.norm(valid_keys, axis=1) + 1e-8
            sims = (valid_keys @ last_h) / (k_norms * h_norm)

            best_idx = np.argmax(sims)
            best_sim = sims[best_idx]

            if best_sim > SIM_THRESHOLD:
                final_pred = int(self.mem_vals[valid_indices[best_idx]])
                mem_override = True

        # Bit accuracy calculation
        tgt_x = np.array([(tgt_byte >> i) & 1 for i in range(8)], dtype=np.float32)
        xb = int(final_pred) ^ tgt_byte
        ...
        if is_learn:
            # 1. Update Transformer core
            d_head = LR * np.outer(err_vec, last_h)
            self.W_head += d_head
            self.tok_embed[in_byte] += LR * (err_vec @ self.W_head)

            # 2. Update Sparse Memory on prediction error
            if final_pred != tgt_byte:
                # Store context key and target value in sparse memory circular buffer
                self.mem_keys[self.mem_ptr] = last_h
                self.mem_vals[self.mem_ptr] = tgt_byte
                self.mem_valid[self.mem_ptr] = True
                self.mem_ptr = (self.mem_ptr + 1) % MEMORY_SLOTS

        return final_pred, err_sum

def run_arm(seed, is_learn):
    patch = build_patch(seed)
    n = len(patch)

    agents = [HybridAgent(seed * 100 + o) for o in range(N_ORGS)]
...
def main():
    results_dir = os.path.join(ROOT, "experiments", "sub5_results")
    os.makedirs(results_dir, exist_ok=True)

    print("=" * 64)
    print("SUBSTRATE 5: HYBRID TRANSFORMER + SPARSE MEMORY RUN")
    print(f"Seeds: {SEEDS} | Ticks: {TICKS} | Report: {REPORT_EVERY} | Orgs: {N_ORGS}")
    print("=" * 64)

    summary = {
        "substrate": "Substrate5_Hybrid_Transformer_Memory",
        "protocol": "SUBSTRATE_5_HYBRID_TRANSFORMER_MEMORY_v1",
        "seeds": SEEDS,
        "ticks": TICKS,
    ...
    out_file = os.path.join(results_dir, "sub5_summary.json")
    with open(out_file, "w") as f:
        json.dump(summary, f, indent=2)

    print("\n" + "=" * 64)
    print("SUBSTRATE 5 SUMMARY & RULE 18 VERDICT")
    print("=" * 64)
