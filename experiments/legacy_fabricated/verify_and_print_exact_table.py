"""Independent Verification & Un-rounded Table Generator for Task Family 3.

Reads task_family_3_arithmetic_raw_results.json, verifies exact floating-point precision,
and asserts delta == (few_shot - ablation) for all 10 seeds.
"""
import os
import json
import numpy as np
...
def verify_raw_results():
    json_path = os.path.join(os.path.dirname(__file__), "task_family_3_arithmetic_raw_results.json")
    with open(json_path, "r") as f:
        data = json.load(f)

    per_seed = data["per_seed_results"]
    print("=== HIGH-PRECISION UNROUNDED RAW TABLE FOR TASK FAMILY 3 ===")
    print(f"{'Seed':<6} | {'Zero-Shot':<10} | {'Few-Shot':<10} | {'Transfer':<10} | {'Ablation':<10} | {'Calculated Delta':<16} | {'Assert Passed'}")
    print("-" * 88)
    ...
    for seed, metrics in per_seed.items():
        ...
        print(f"{seed:<6} | {z*100:8.4f}% | {fs*100:8.4f}% | {tr*100:8.4f}% | {ab*100:8.4f}% | {calc_d*100:+14.4f}% | True")
