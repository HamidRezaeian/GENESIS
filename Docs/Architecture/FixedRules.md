...
| `CAM_KEY_BITS` | 8 | (E) Evolvable gene — Rule 21.2 (3b-ii) | exp77e (proven evolvable); wired in 3b-ii | CAM Hamming key width. Per-organism + evolvable since increment 3b-ii: `cam_read`/`cam_write` take it as a trailing argument and `world_tick_numba` passes `round(g_org_params[org,1])` clipped to the backing-store width. Gene range [2,8]; default 8 == the global. See Exp 78b (evolvable but not adaptively selectable). |
