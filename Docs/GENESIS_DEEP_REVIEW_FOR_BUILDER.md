...
> داوری نهاییِ تأییدی ثبت شد؛ نتیجه در Result.md Exp 94(c).
> ⛔ **همان جلسه — Exp 95 (کشف بزرگ):** ممیزی 92b روی کل `experiments/` یک **جزیره‌ی جعل**
> root-cause کرد: درایورهای Phase-D/E/F/G و Task-Family ۲ تا ۵ و زنجیره‌ی replication/1200
> هیچ ارزیابی واقعی انجام نمی‌دادند — دقت = ثابتِ هاردکد + jitter تصادفی، و حتی p-valueهای
> Wilcoxon/permutation هم هاردکد بودند (۰٫۰۰۱۹۵۳/۰٫۰۰۰۹۷۶)؛ «Replication Engine» تابع
> ارزیابیِ همان فابریکه‌ی فاز E را import می‌کرد و CI دو تستِ «نگهبان» این زنجیره داشت.
> ۴۰ فایل (۱۸ اسکریپت + ۲۲ آرتیفکت) به `experiments/legacy_fabricated/` منتقل شدند
> (+ ۲ تست به tests/legacy/)، نگهبانِ جدید `tests/fabrication_scan_test.py` در fast suite
> فعال است، و هر ۷ سند PROTOCOLS بنر اصلاحیه گرفتند. سطوح leaderboard واقعی (TF1) و داشبورد
> هرگز از این زنجیره تغذیه نشده بودند. جزئیات: Result.md Exp 95. نتیجه‌ی استراتژیک:
> **تنها سطرِ سنجیده‌شده‌ی معیار B/C برنامه‌ی صعود، امروز TF1 است.**
> ✅ **Session 17 (همان روز):** دو اصلاحیه‌ی بدهکارِ ممیزی ثبت شد (روش MC به‌جای «شمارش دقیق»
> در 94b؛ تجزیه‌ی Ark در 92-M). سپس **Exp 96** (نقشه‌ی اکتشافی DIV×تمپو، ۱۴ ترکیب، پیش‌ثبت):
> H1 پشتیبانی نشد؛ نامزدها: default|div32 و fast|div1 — و دامِ winner's-curse در بندِ reuse
> پیش‌ثبت‌شده شناسایی و پیش از اجرا اصلاح شد (تأییدی فقط با seedهای تازه‌ی 24..47 و Bonferroni
> α=۰٫۰۲۵). **Exp 97 (تأییدی، pieces 98 اجرای زنده): هر دو هدف شکست خوردند** — div32:
> Δ=−۱٫۴۹ (علامت برعکس شد!), p=۰٫۰۷۵؛ fast: Δ=+۰٫۲۰، p=۰٫۸۴. طبق بندِ پیش‌ثبت‌شده،
> **محور تنظیم برای STDP3Cِ خام بسته شد**؛ گام بعدی تغییرِ مکانیسم است (Exp 98: پلاستیسیته‌ی
> گِیت‌شده روی تعجب؛ Exp 99: تثبیتِ دوروکشی)، هر دو فقط از دروازه‌ی 92b عبور می‌کنند.
> جزئیات: Result.md Exp 96/97. حکم استراتژیک به‌روز: **Rule-18-B به‌صورت اثبات‌نشده ولی
> آموزنده باقی است؛ مسیرِ بعدی مکانیسم است نه پیچ‌چرخانی.**
> ✅ **Session 18 (همان روز):** تغییرِ مکانیسمِ پیش‌ثبت‌شده اجرا شد — **Exp 98: پلاستیسیته‌ی
> گِیت‌شده روی تعجب** (`dopamine = net − میانگین دوره‌ای net`؛ افق = ساعتِ REMAP_PERIODِ
> خودِ محیط، بدون ثابت جدید). پیش از اولین سطرِ سنجش، دو نقص ابزاریِ خودساخته شکار و
> نگهبان‌دار شدند: (۱) کلید جدید در تاپلِ KERNEL_STATE_VARS نبود → کرنلِ کهنه‌ی numba بین
> بازوها مشترک شد (کلاس Session-11)؛ (۲) نام‌گذاریِ فایل‌های خام دو بازو برخورد می‌کرد.
> **نتیجه‌ی الزام‌آور: اولیه شکست خورد** — gated در برابر NOLEARN: Δ=+۲٫۲۲، p=۰٫۲۱۹
> (15/24 مثبت)، CONFIRMED=False. ثانویه‌ها (بدون مصرف آلفا): گیت روی عملکردِ تکلیف خنثی است
> (Δ=+۰٫۱۵، p=۰٫۹۴) ولی فرسایشِ حافظه‌ی ایستا را معنادار کم می‌کند (Δ=+۱٫۱۴، p=۰٫۰۰۰۱) —
> هرچند نه تا آستانه‌ی ثبت‌شده‌ی ≥۹۵ (۹۳٫۲) و NOLEARN همچنان بالاتر است (۹۴٫۰). حکم:
> فرضیه‌ی گیتینگ **در این مکان بسته شد**؛ کنترلِ «کی شلیک کند» مزیتِ یادگیری را برنمی‌گرداند.
> گام بعدی طبق بند: **Exp 99 — تثبیتِ دوروکشی** با باندِ ایستای ≥۹۵ به‌عنوان دروازه‌ی گواهی.
> قاعده‌ی میراثی ثبت شد: هر مکانیسمِ پرچم‌دارِ جدید پیش از اولین سطرِ سنجش باید در تاپل+نگاشتِ
> fingerprint و اثباتِ واگراییِ smoke بنشیند. جزئیات: Result.md Exp 98.
> نگاشتِ claim-to-evidence این چهار Session برای داوریِ مستقل: `Docs/REVIEW_PACK.md`.
>
> ⚠️ **بازمانده:**
> P1‑4 ✅ برای TF1 بسته شد: داوری 94b (n=24, پیش‌ثبت‌شده) اجرا و ثبت شد — **پوک (NULL):
> mean +۱٫۴۳، p=۰٫۶۰۶**؛ گام بعدی نقشه‌ی پایداری/انعطاف‌پذیری به‌جای seed بیشتر. خانواده‌های
> ۲ تا ۵ نیازمند درایور سنجیده‌شده‌ی جدید از صفرند (درایورهای قبلی جعلی اعلام و قرنطینه شدند — Exp 95),
> P1‑3 (کنترل‌های کامل learning), P1‑5 (shortcut audit خودکار),
...
> جزئیات هر فاز در `Docs/RESUME_NEXT_SESSION.md` (Session 15 و 16).
>
