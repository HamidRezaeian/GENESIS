# lava_network_v9.py - کاهش firing rate با حفظ amplification
"""
GENESIS Phase 2 — Firing Rate Reduction Sweep
Weight=3 ثابت، vth/dv/input_sparsity متغیر
"""
...
def run_experiment(weight, vth, connectivity, input_sparsity, dv, du, label=""):
...
def main():
    print("=" * 70)
    print("GENESIS Phase 2 — Firing Rate Reduction Sweep")
    print("Goal: Weight=3 ثابت، کاهش firing rate با vth/dv/input_sparsity")
    print("=" * 70)
    
    # Sweep: vth × dv × input_sparsity با weight=3
    configs = [
        # (weight, vth, connectivity, input_sparsity, dv, du, label)
        (3, 20, 0.10, 0.05, 0.3, 0.3, "Baseline (w=3)"),
        (3, 30, 0.10, 0.05, 0.3, 0.3, "vth=30"),
        (3, 40, 0.10, 0.05, 0.3, 0.3, "vth=40"),
        (3, 50, 0.10, 0.05, 0.3, 0.3, "vth=50"),
        (3, 30, 0.10, 0.05, 0.5, 0.5, "vth=30, dv=0.5"),
        (3, 40, 0.10, 0.05, 0.5, 0.5, "vth=40, dv=0.5"),
        (3, 30, 0.10, 0.04, 0.3, 0.3, "vth=30, input=4%"),
        (3, 30, 0.10, 0.03, 0.3, 0.3, "vth=30, input=3%"),
        (3, 40, 0.10, 0.04, 0.5, 0.5, "vth=40, dv=0.5, input=4%"),
        (3, 50, 0.10, 0.04, 0.5, 0.5, "vth=50, dv=0.5, input=4%"),
    ]
    
    results = []
    for weight, vth, conn, inp_sp, dv, du, label in configs:
        print(f"\n🔬 Testing: {label}")
        
        try:
            result = run_experiment(weight, vth, conn, inp_sp, dv, du, label)
    ...
    print("📊 SUMMARY TABLE:")
    print(f"{'='*70}")
    print(f"{'Config':<30} {'Sparsity':<10} {'Amp':<8} {'Rate':<8} {'Status'}")
    print(f"{'-'*70}")
    ...
    for r in results:
        ...
        rate_ok = 1 <= r['firing_rate'] <= 40  # کمی بالاتر از 30
        ...
        print(f"{r['label']:<30} {r['sparsity']:<10.2%} {r['amplification']:<8.2f} {r['firing_rate']:<8.1f} {status}")
    ...
    if best_config:
        ...
        print(f"   Input sparsity:  {best_config['input_sparsity']:.0%}")
        print(f"   Voltage decay:   {best_config['dv']}")
        print(f"   Current decay:   {best_config['du']}")
        print(f"\n   Output sparsity: {best_config['sparsity']:.2%}")
