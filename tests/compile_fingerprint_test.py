...
_CHILD = (
    ...
    "m = np.zeros(8, np.bool_); "                      # force ONE kernel compilation so the
    "ne.malloc_block(2, m); "                          # cache locator must bind somewhere
...
def main():
    ...
    print(f"[1] Engine Env-Read Coverage OK: {len(covered)} GENESIS_* reads mapped, 0 uncovered")
    ...
    for env_name, reason in cfp.HOST_SIDE_EXEMPT.items():
        assert isinstance(reason, str) and len(reason) > 10, f"{env_name} exempt without a reason"
    ...
    fake2 = dict(fake); fake2["RAM_SIZE"] = 2
    ...
    d_ram = _child_cache_dir({"GENESIS_RAM_SIZE": "65536"})
    ...
    env = dict(os.environ); env["NUMBA_CACHE_DIR"] = "/tmp/genesis_user_explicit_cache"
    src_dir = os.path.dirname(ENGINE_PATH)
    out = subprocess.run([sys.executable, "-c", _CHILD % src_dir],
                         capture_output=True, text=True, env=env, timeout=600)
    assert out.returncode == 0, out.stderr[-2000:]
    assert out.stdout.strip().splitlines()[-1] == "/tmp/genesis_user_explicit_cache"
