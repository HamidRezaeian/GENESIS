"""
Experiment 87: First Genuine Multi-Generational Selection Benchmark (AUTO_REPRO)
(Rule 2, Rule 3, Rule 14, Rule 16, Rule 18, Rule 20)
================================================================================
2-arm x 5-seed x 10,000 ticks. AUTO_REPRO=1 enables energy-gated physical fission.

Arm 1 (Control):  AUTO_REPRO=1, MULTISCALE=0, STDP3C=0
Arm 2 (Proposed): AUTO_REPRO=1, MULTISCALE=1, STDP3C=1

Measures: Mean Pop, Mean Energy, total_births, birth_rate, Std, Delta, Z-score.
"""
...
AUTO_REPRO_THRESH = "150000.0"

ARMS = [
    {
        "name": "Arm 1: Control (AutoRepro, Reflex)",
        "env": {
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
    ...
    {
        "name": "Arm 2: Proposed (AutoRepro, MULTISCALE+STDP3C)",
        "env": {
            "GENESIS_DIGESTION": "1",
            "GENESIS_PEER": "1",
...
def main():
    worker_script = os.path.join(REPO_ROOT, "exp87_worker.py")
    with open(worker_script, "w", encoding="utf-8") as f:
        f.write(SUBWORKER_CODE)

    print("==========================================================================")
    print("Experiment 87: First Genuine Multi-Generational Selection (AUTO_REPRO)")
    print("==========================================================================")
    ...
    for arm_name, seed_runs in results.items():
        ...
        print(f"{arm_name[:44]:<44}: Pop={arm_stats[arm_name]['mean_pop']:.2f}±{arm_stats[arm_name]['std_pop']:.2f} | E={arm_stats[arm_name]['mean_energy']:.0f} | Births={arm_stats[arm_name]['mean_births']:.1f}±{arm_stats[arm_name]['std_births']:.1f}")

    arm1 = arm_stats["Arm 1: Control (AutoRepro, Reflex)"]
    arm2 = arm_stats["Arm 2: Proposed (AutoRepro, MULTISCALE+STDP3C)"]
    ...
    z_births = delta_births / arm2["std_births"] if arm2["std_births"] > 0 else 0.0

    print("--------------------------------------------------------------------------")
    print(f"Arm2 vs Arm1: ΔPop={delta_pop:+.2f}, Z_pop={z_pop:+.2f}σ | ΔBirths={delta_births:+.1f}, Z_births={z_births:+.2f}σ")

    births_nonzero = arm1["mean_births"] > 0 and arm2["mean_births"] > 0
    passed = (delta_pop > 0) and (z_pop >= 1.0) and births_nonzero

    if not births_nonzero:
        verdict = "SURVIVORSHIP CONFOUND — births=0, founder persistence only (Rule 14/16)"
    elif passed:
        verdict = "MULTI-GENERATIONAL ASCENT CONFIRMED (ΔPop Z≥1σ, births>0)"
    else:
        verdict = f"NULL — births>0 but ΔPop Z={z_pop:+.2f}σ < 1.0σ threshold"

    print(f"BINDING VERDICT (Rule 2/14/16/18): {verdict}")

    out_file = os.path.join(REPO_ROOT, "exp87_results.json")
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump({"arms": results, "stats": arm_stats,
                   "delta_pop": delta_pop, "z_pop": z_pop,
                   "delta_births": delta_births, "z_births": z_births,
