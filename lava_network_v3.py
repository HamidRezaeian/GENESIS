# lava_network_v3.py - با buffer برای Sink
"""
GENESIS Phase 2 — Complete Event-Driven Network (v3)
Sink with proper buffer parameter
"""
...
def main():
    print("=" * 70)
    print("GENESIS Phase 2 — Complete Event-Driven Network (v3)")
    print("=" * 70)
    ...
    source = Source(data=input_data.T)  # [n_neurons, n_steps]
    
    # Dense: synaptic connections
    weights = np.random.rand(n_neurons, n_neurons).astype(int)
    ...
    # Sink: collect output (با buffer برای ذخیره داده‌ها)
    output_buffer = np.zeros((n_neurons, n_steps), dtype=int)
    sink = Sink(buffer=output_buffer)
    ...
    try:
        ...
        sink_output = output_buffer  # Direct access to buffer
        total_output_spikes = int(np.sum(sink_output))
        active_neurons = int(np.sum(np.sum(sink_output, axis=0) > 0))
    ...
    if 0.01 <= output_sparsity <= 0.15:
        ...
        print(f"   ⚠️  Too sparse — increase weight or lower threshold")
    else:
